...
export class ServiciosComponent implements OnInit {
  ...
  displayedColumns: string[] = ['id', 'fecha_ingreso', 'nombre_cliente', 'producto', 'marca', 'encargado', 'cotizacion', 'fecha_terminado', 'fecha_entrega', 'estatus', 'acciones'];
  ...
  getServicios(){
    this.mainService.requestMany({ _function: "fnGetServiciosPorMes", mes: this.date.value.month() + 1, ano: this.date.value.year() }, this.model).subscribe((data: ServiciosRes[])=>{
  ...
  deleteServicio(id: String, nombre: String){
    Swal.fire({
      ...
      if (result.value) {
        this.mainService.requestOne({ _function: "fnDeleteServicio", id: id }, this.model).subscribe((data)=>{console.log(data)});
        this.getServicios();
